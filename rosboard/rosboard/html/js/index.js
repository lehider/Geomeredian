...
let onSystem = function(system) {
  if(system.hostname) {
    ...
    $('.mdl-layout-title').text("Meridian Geomatics & Engineering");
  }
...
let onTopics = function(topics) {
  ...
  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher1({name: "lidar_control", messageType: "std_msgs/String"});  })
  .text("Record lidar data")
  .appendTo($("#lidar-nav"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher2({name: "lidar_control", messageType: "std_msgs/String"}); })
  .text("Save lidar data")
  .appendTo($("#lidar-nav"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher3({name: "GNSS_control", messageType: "std_msgs/String"});  })
  .text("Record GNSS data")
  .appendTo($("#gnss-nav"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher4({name: "GNSS_control", messageType: "std_msgs/String"}); })
  .text("Save GNSS data")
  .appendTo($("#gnss-nav"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher1({name: "MAP_control", messageType: "std_msgs/String"});  })
  .text("Record map")
  .appendTo($("#map-nav"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initPublisher2({name: "MAP_control", messageType: "std_msgs/String"}); })
  .text("Save map")
  .appendTo($("#map-nav"));

}

function initPublisher1({topicName, topicType}) {
  var testStr = new ROSLIB.Topic({   ros : ros,
    name : 'lidar_control',
...
function initPublisher2({topicName, topicType}) {
  var testStr = new ROSLIB.Topic({ros : ros,
    name : 'lidar_control',
    messageType : 'std_msgs/String'});
  var str = new ROSLIB.Message({ data:'stop'});
  testStr.publish(str);
  console.log(testStr);
}


function initPublisher3({topicName, topicType}) {
  var testStr = new ROSLIB.Topic({   ros : ros,
    name : 'gps_control',
    messageType : 'std_msgs/String'});
  var str = new ROSLIB.Message({ data:'start'});
...
function initPublisher4({topicName, topicType}) {
  var testStr = new ROSLIB.Topic({ros : ros,
    name : 'gps_control',
    messageType : 'std_msgs/String'});
  var str = new ROSLIB.Message({ data:'stop'});
